...
class UploadTaskViewController: UIViewController {
   ...
   @IBAction func uploadWithProgress(_ sender: Any) {
      ...
      // Code Input Point #3
       ...
       // 1. url 생성
       // 2. url 사용해서 request 생성
       // 3. session configuration 생성 및 세션 생성
       // 4. session.task -> 태스크 생성
      // Code Input Point #3
   }
   ...
   @IBAction func upload(_ sender: Any) {
      ...
      // Code Input Point #1
       let task = URLSession.shared.uploadTask(with: dropboxUploadRequest, from: data) { data, respoonse, error in
           if let error = error{
